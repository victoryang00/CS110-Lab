...
//solve by reduction
double dotp_red(double* x, double* y) {
    double global_sum = 0.0;

    #pragma omp parallel for reduction (+:global_sum)
    //why there cannot be kuohao
    for(int i=0; i<ARRAY_SIZE; i++){
        global_sum += x[i] * y[i];}
    return global_sum;
}
...
int main() {
	...
	for(int i=1; i<=num_threads; i++) {
		...
		run_time = omp_get_wtime() - start_time;
  		printf(" %d thread(s) took %f seconds, originally\n",i,run_time);

  		//below is the reduction optimization
        start_time = omp_get_wtime();
        for(int j=0; j<REPEAT; j++)
            result = dotp_red(x,y);
        run_time = omp_get_wtime() - start_time;
        printf(" %d thread(s) took %f seconds, reductively\n\n",i,run_time);
