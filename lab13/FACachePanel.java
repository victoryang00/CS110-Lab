...
class FACachePanel extends JPanel {
	...
	public void paintComponent(Graphics g){
		...
		*/
		...
		//ACCOUNT FOR 36 PIXELS FOR THE BLOCK LABEL, 40 FOR THE TAG, THEN THE REST IS FOR THE 8 WORDS PER BLOCK
		dx = (int) ((dC.width-76)/8);    //8 WORDS PER BLOCK
		int temp1 = dx*8;
		offsetX = (int)((dC.width - 76 - temp1)/2);   //USED TO HORIZONTALLY CENTER THE DRAWING IN THE PANEL
		...
		for (int i = 0; i < 16; i++){
			...
			g.drawLine(offsetX+36+10, offsetY+15+dy*i+dy/2, 76+10+offsetX, offsetY+15+dy*i+dy/2);
		...
		g.drawLine(offsetX+76+10, offsetY+15+dy*16, dC.width+10-offsetX, offsetY+15+dy*16);
		
		for (int i = 0; i < 9; i++) {

			//VERTICAL LINES TO DIVIDE THE BLOCKS INTO WORDS
			g.drawLine(offsetX+76+10+dx*i, offsetY+15, offsetX+76+10+dx*i, dC.height+15-offsetY);
		}//END FOR
		...
		for (int i = 0; i < 16; i++){
			if (boolBlocks[i]){
				...
				g.fillRect(offsetX + 37 +10, offsetY + dy*i + 1+15, 39, dy/2 - 1);
				g.setColor(Color.yellow);

				//HIGHLIGHT THE BLOCK
				for (int j = 0; j < 8; j++){
					g.fillRect(77 + 10 + offsetX + dx*j, 15 + offsetY + dy*i + 1, dx - 1, dy - 1);

					//HIGHLIGHT THE WORD
					if (boolWords[j] == true){
						g.setColor(Color.red);
						g.fillRect(77 + offsetX + dx*j+10, offsetY + dy*i + 1+15, dx-1, dy-1);
						g.setColor(Color.yellow);
					}
				}
			}
			...
			if (!stringBlocks[i].equals("")){
				g.setColor(Color.black);
				for (int j = 0; j < 8; j++){
					g.drawString("B"+stringBlocks[i],78+10+offsetX+dx*j, offsetY+dy*i+15+15);
					g.drawString("W"+j, 78+10+offsetX+dx*j+2, offsetY+dy*i+26+15);
				}
				g.drawString(tag[i], offsetX+40+10, offsetY+dy*i+15+15);
			}
		}
		
	}//END FUNCTION paintComponent

}//END CLASS FACachePanel